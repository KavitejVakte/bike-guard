...
const resolveApiBase = () => {
  if (window.BIKEGUARD_API_BASE) return window.BIKEGUARD_API_BASE
  if (window.location.protocol === 'file:') return ''
  if (window.location.hostname === 'bike-guard.vercel.app') {
    return 'https://bike-guard-api.onrender.com/api'
  }
...
const apiFetch = async (path, options = {}) => {
  if (!API_BASE) {
    throw new Error(
      'Server not reachable. Please run the backend and open the app via http://<your-ip>:3000 (not as a file).'
    )
  }
  const headers = {
