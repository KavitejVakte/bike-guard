...
const COOKIE_NAME = 'bikeguard_token'
...
const CORS_ORIGINS = (process.env.CORS_ORIGIN || 'http://localhost:3000,https://bike-guard.vercel.app')
  .split(',')
  .map((entry) => entry.trim())
  .filter(Boolean)

// Credentialed CORS is required for cookie-based auth across origins.
app.use(
  cors({
    origin: CORS_ORIGINS,
    credentials: true,
...
// Cross-site cookie auth requires SameSite=None + Secure in production.
const cookieOptions = {
  httpOnly: true,
  secure: IS_PROD,
