...
const { kv } = KV_ENABLED ? require('@vercel/kv') : { kv: null }
const DEFAULT_DB = { users: [], rides: [], issues: [], sosEvents: [] }
...
const readDb = async () => {
  if (KV_ENABLED) {
    const stored = await kv.get('bikeguard:db')
    return stored ? stored : cloneDb(DEFAULT_DB)
  }
...
app.use(async (req, res, next) => {
  try {
    if (process.env.VERCEL && !KV_ENABLED) {
      return res.status(500).json({
        error:
          'Storage not configured. Set Vercel KV environment variables or configure a persistent database.'
      })
    }
    await ensureSeeded()
    return next()
...
app.put('/api/rides/:id', auth, async (req, res) => {
  const { id } = req.params
  const { name, durationMin, distanceKm, type, safetyScore } = req.body || {}
  const db = await readDb()
...
app.post('/api/tracking/stop', auth, async (req, res) => {
...
app.get('/api/export/rides.csv', auth, async (req, res) => {
  const db = await readDb()
  const rides = db.rides.filter((r) => r.userId === req.userId)
...
app.get('/api/export/issues.csv', auth, async (req, res) => {
  const db = await readDb()
  const issues = db.issues.filter((i) => i.userId === req.userId)
...
app.get('/api/export/summary.pdf', auth, async (req, res) => {
  const db = await readDb()
  const user = db.users.find((u) => u.id === req.userId)
  const rides = db.rides.filter((r) => r.userId === req.userId)
  const issues = db.issues.filter((i) => i.userId === req.userId)
